from mihto.evaluator.exceptions import NoExpressionException, NoEnvironmentalVariableFoundException, OperatorMismatchException
...
class Evaluator:
    ...
    def evaluate(self, expression: ExpressionNode) -> bool:
        if expression is None:
            raise NoExpressionException("No expression was given to evaluate")
        ...
        if type(expression.value1) is ExpressionNode or type(expression.value2) is ExpressionNode:
            raise OperatorMismatchException("Operator mismatch. Cannot perform {} operator with boolean data".format(operator))
        ...
        if type(value1) is str:
            ...
            if value1 is None:
                raise NoEnvironmentalVariableFoundException("{} cannot be found in environment {}".format(value1, self.env))
